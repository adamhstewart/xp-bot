...
def setup_info_commands(bot, db, guild_id):
    ...
    async def xp_help(interaction: discord.Interaction):
        ...
        embed.add_field(name="/xp", value="View characters with navigation. Set active, retire, or view other users.", inline=False)
        embed.add_field(name="/xp_edit", value="Edit character name, image, or sheet URL.", inline=False)
        embed.add_field(name="/xp_request", value="Request XP for a character (admin approval required).", inline=False)
        embed.add_field(name="/xp_retire", value="(Admin) Retire a character (soft delete, can be restored).", inline=False)
